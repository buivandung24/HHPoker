...
public class SendMeberDiamondTopPanel : BasePlane {
    ...
    private void Awake()
    {
        ...
        sendToggle.onValueChanged.AddListener((isOn) =>
        {
            if (isOn)
            {
                ...
                NetMngr.GetSingleton().Send(InterfaceClub.GetPlayerDiamondInfo, new object[] { ClubManager.GetSingleton().clubPanel.clubId });
            }
        });
    ...
    public void GetMeberDiamondTopCallBack(Hashtable data)
    {
        this.sendText.text = data["canSendDiamond"].ToString();
        this.aleardyText.text = data["aleardySendDiamond"].ToString();
    }
    ...
    public void SendTo()
    {
        ...
        {
            NetMngr.GetSingleton().Send(InterfaceClub.SendToPlayerDiamond, new object[] { ClubManager.GetSingleton().clubPanel.clubId, inputName.text, inputCount.text });
            inputName.text = "";
            inputCount.text = "";
        }
        else
    ...
    {
        NetMngr.GetSingleton().Send(InterfaceClub.GetPlayerDiamondrecord, new object[] {ClubManager.GetSingleton().clubPanel.clubId });
    }
    ...
    public void GetRedcordInfoCallback(Hashtable data)
    {
        ...
        for (int i = 0; i < withList.Count; i++)
        {
            ...
            zengsongItemd.SetData(withList[i] as Hashtable);
        }
    }
