...
public class MyDataTopPanel : BasePlane {
    ...
    private void Awake()
    {
        ...
        topGroup[0].onValueChanged.AddListener((isOn) =>{if (isOn){TopShow(0);}});
        topGroup[1].onValueChanged.AddListener((isOn) => { if (isOn) { TopShow(1); } });
        topGroup[2].onValueChanged.AddListener((isOn) => { if (isOn) { TopShow(2); } });
        topGroup[3].onValueChanged.AddListener((isOn) => { if (isOn) { TopShow(3); } });
        middleGroup1[0].onValueChanged.AddListener((isOn) =>{if (isOn){MiddleShow1(0);}});
        middleGroup1[1].onValueChanged.AddListener((isOn) => { if (isOn) { MiddleShow1(1); } });
        middleGroup1[2].onValueChanged.AddListener((isOn) => { if (isOn) { MiddleShow1(2); } });
        middleGroup2[0].onValueChanged.AddListener((isOn) =>{if (isOn){MiddleShow2(0);}});
        middleGroup2[1].onValueChanged.AddListener((isOn) => { if (isOn) { MiddleShow2(1); } });
        middleGroup2[2].onValueChanged.AddListener((isOn) => { if (isOn) { MiddleShow2(2); } });
    }
